...
def make_prompt(row):
    return (
        "You are a helpful biomedical assistant. Answer the question strictly with a single letter: 'A', 'B', 'C', 'D' or 'E'.\n"
...
for i, (prompt, expected) in enumerate(zip(df["prompt"], df["answer_idx"])):
    if i >= total:
        break
    ...
    match = re.search(r"\b([A-E])\b", after_answer)
    if match:
        predicted = match.group(1)
    else:
        fallback_match = re.search(r"[A-E]", after_answer)
