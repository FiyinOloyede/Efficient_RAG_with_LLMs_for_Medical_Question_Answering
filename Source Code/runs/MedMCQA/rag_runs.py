...
# Build the final prompt with few shot prefix
prompt = PromptTemplate(
    input_variables=["context", "question", "opa", "opb", "opc", "opd"],
    template=f"""
{prefix}
...
for i, row in df.iterrows():
    ...
    match = re.search(r"\b([A-D])\b", generated_part)
    if match:
        predicted = match.group(1)
    else:
        fallback_match = re.search(r"[A-D]", generated_part)
    ...
    if i < 5:
        ...
        print(f"\nExpected: {row['cop']}, Predicted: {predicted}")
